...
from src.model.qmixer import QMixerModel
...
class QMIX:
    def __init__(
        ...
        assert hasattr(qmixer_model, "n_agents") and not callable(
            getattr(qmixer_model, "n_agents", None)
        ), "qmixer_model needs to have attribute n_agents"
        ...
        self.target_agent_model.to(self.device)
        self.qmixer_model.to(self.device)
        self.target_qmixer_model.to(self.device)
    ...
    def init_hidden_states(self, batch_size: int) -> None:
        self.hidden_states = (
            self.agent_model.init_hidden()
            .unsqueeze(0)
            .expand(batch_size, self.n_agents, -1)
            .to(self.device)
        )
        self.target_hidden_states = (
            self.target_agent_model.init_hidden()
            .unsqueeze(0)
            .expand(batch_size, self.n_agents, -1)
            .to(self.device)
        )
    ...
    def learn(
        ...
        for t in range(episode_len):
            ...
            local_q, self.hidden_states = self.agent_model(obs, self.hidden_states)
        ...
        target = (
            reward_batch + self.gamma * (1 - terminated_batch) * target_global_max_qs
        )
