...
Each agent can just observe itself (its own identity) i.e. s_j = j and vision square around it.
...
class PredatorPreyEnv:
    ...
    def __init__(self):
        ...
        self.mode = None
        self.enemy_comm = None
        self.stat = None
        self.moving_prey = None
        self.prey_loc = None
        self.obs = None
    ...
    def init_args(self, parser: argparse.ArgumentParser):
        env = parser.add_argument_group("Prey Predator task")
        env.add_argument(
            "--nenemies", type=int, default=1, help="Total number of preys in play"
        )
        # env.add_argument('--n_agent', type=int, default=3,
    ...
    def multi_agent_init(self, args: argparse.Namespace):
    ...
    def reset(self):
        ...
        self.predator_loc, self.prey_loc = (
            locs[: self.npredator],
            locs[self.npredator :],
        )
    ...
    def _get_cordinates(self):
        idx = np.random.choice(
            np.prod(self.dims), (self.npredator + self.nprey), replace=False
        )
    ...
    def _set_grid(self):
        ...
        # Padding for vision
        self.grid = np.pad(self.grid, self.vision, constant_values=self.OUTSIDE_CLASS)
    ...
    def _take_action(self, idx, act):
        ...
        if (
            act == 0
            and self.grid[
                max(0, self.predator_loc[idx][0] + self.vision - 1),
        ...
        # RIGHT
            ...
            act == 1
            and self.grid[
                self.predator_loc[idx][0] + self.vision,
                min(self.dims[1] - 1, self.predator_loc[idx][1] + self.vision + 1),
            ]
            != self.OUTSIDE_CLASS
        ):
            self.predator_loc[idx][1] = min(
                self.dims[1] - 1, self.predator_loc[idx][1] + 1
            )
        ...
        # DOWN
            ...
            act == 2
            and self.grid[
                min(self.dims[0] - 1, self.predator_loc[idx][0] + self.vision + 1),
                self.predator_loc[idx][1] + self.vision,
            ]
            != self.OUTSIDE_CLASS
        ):
            self.predator_loc[idx][0] = min(
                self.dims[0] - 1, self.predator_loc[idx][0] + 1
            )
        ...
        # LEFT
            ...
            and self.grid[
                self.predator_loc[idx][0] + self.vision,
                max(0, self.predator_loc[idx][1] + self.vision - 1),
            ]
            != self.OUTSIDE_CLASS
        ):
    ...
    def _get_reward(self):
        ...
        if self.mode == "cooperative":
            ...
            raise RuntimeError(
                "Incorrect mode, Available modes: [cooperative|competitive|mixed]"
            )
            ...
            for idx, item in enumerate(row):
                if item != 0:
                    if "X" in item and "P" in item:
                        self.stdscr.addstr(
                            row_num, idx * 4, item.center(3), curses.color_pair(3)
                        )
                    elif "X" in item:
                        self.stdscr.addstr(
                    ...
                    self.stdscr.addstr(
                        row_num, idx * 4, "0".center(3), curses.color_pair(4)
                    )
