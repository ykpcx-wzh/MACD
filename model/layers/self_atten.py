...
class SelfAttentionNonlinear(nn.Module):
    def __init__(self, input_size, heads, embed_size):
        super().__init__()
        self.input_size = input_size
        self.heads = heads
    ...
    def forward(self, x):
        ...
        keys = self.act(self.tokeys(x).view(b, t, h, e))
        queries = self.act(self.toqueries(x).view(b, t, h, e))
        values = self.act(self.tovalues(x).view(b, t, h, e))
        ...
        # row wise self attention probabilities
        dot = F.softmax(dot, dim=2)
        out = self.act(torch.bmm(dot, values).view(b, h, t, e))
        out = out.transpose(1, 2).contiguous().view(b, t, h * e)
        return out
    

class CADPSelfAttention(nn.Module):
    def __init__(self, input_size, heads, embed_size):
        super().__init__()
        self.input_size = input_size
        self.heads = heads
